...
|---|---|---|
| 🟢 verified                  | 81 | — |
| 🟡 unverified (oracle gap)    |  5 | Documented in §3 below; tracked for future bench expansion.  Phase 2A closed 4 (LIBPDeriv multi-invariant 2026-05-09; Jordan block ordering, analyze_block non-nested overlapping, Calcx00 boundary linear-system selection 2026-05-10).  Phase 2B closed 4 (`r = nonzero(top) + IBPDot`, MasterRank/MasterDot non-exposure, SingleMassQ Numeric enhancement, factorize_family mass=−1 Numeric enhancement 2026-05-10/11).  Audit cleanup 2026-05-11: 3 rows whose implementations landed in Phase 1A (`SolveIntegrals` single-eps fast path, per-system `AMFSystemDirection`, Cutkosky physical-mass safety check) promoted to 🟢 with explicit Phase 1A attribution.  The Calcx00 acb-inverse fallback (rank-deficient case) remains 🟡 pending a synthetic rank-deficient bench in Phase 3. |
...
| Acb-inverse failure fallback in `Calcx00` | `src/ode/zero.cpp:1199-1206` | C++ degrades to "all positions resonate"; upstream's symbolic inverse never fails.  Still 🟡 — no production-scale bench has been observed to trigger this fallback, so the degraded-but-conservative behavior is empirically safe but not parity-validated.  Phase 3 candidate. |
