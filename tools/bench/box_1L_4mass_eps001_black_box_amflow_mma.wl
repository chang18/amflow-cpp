...
(*
  1-loop box with all 4 propagators carrying distinct masses
  (mAsq, mBsq, mCsq, mDsq).  Fully massive 1L 4-leg.  Target:
  corner j[box4m, 1, 1, 1, 1] at eps = 1/1000.
*)
...
AMFlowInfo["Numeric"] = {
  s -> -3, t -> -1, mAsq -> 1, mBsq -> 4, mCsq -> 9, mDsq -> 16
};
AMFlowInfo["NThread"] = 4;

targets = { j[box4m, 1, 1, 1, 1] };
epslist = {1/1000};
cache = Environment["AMF_BENCH_MMA_CACHE"];
If[cache === $Failed || cache === None,
  cache = FileNameJoin[{repo, "tools", "bench", "mma_refs",
    "box_1L_4mass_eps001_mma_cache"}]];

Print["==BENCH== mma 1-loop box with 4 distinct masses"];
Print["==CACHE== ", cache];
