...
(*
  2-loop xbox (crossed box) with 2 distinct masses on the two loop
  propagators (mAsq on l1, mBsq on l2).  First multi-mass xbox
  oracle.  Target: corner j[xbox2m, 1,1,1,1,1,1,1,0,0] at eps = 1/1000.
*)
...
AMFlowInfo["Numeric"] = {s -> -3, t -> -1, mAsq -> 1, mBsq -> 4};
AMFlowInfo["NThread"] = 4;

targets = { j[xbox2m, 1, 1, 1, 1, 1, 1, 1, 0, 0] };
epslist = {1/1000};
cache = Environment["AMF_BENCH_MMA_CACHE"];
If[cache === $Failed || cache === None,
  cache = FileNameJoin[{repo, "tools", "bench", "mma_refs",
    "xbox_2L_2mass_eps001_mma_cache"}]];

Print["==BENCH== mma 2-loop crossed box with 2 distinct loop masses"];
Print["==CACHE== ", cache];
