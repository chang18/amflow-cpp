...
(*
  ...
  Topology: NPB topology adapted from Smirnov-Veretin xbox routing with uniform electron mass (Bhabha non-planar double-box). Middle rung (l1+l2)^2 distinguishes NPB from PB.
...
targets = { j[bhabha2npb, 1, 1, 1, 1, 1, 1, 1, 0, 0] };
epslist = {1/100000};
cache = Environment["AMF_BENCH_MMA_CACHE"];
If[cache === $Failed || cache === None,
  cache = FileNameJoin[{repo, "tools", "bench", "moller_scalar_qed", "mma_refs",
    "bhabha2L_NPB_physical_eps00001_mma_cache"}]];

Print["==BENCH== mma bhabha 2L non-planar double-box (NPB; crossed middle rung), Euclidean region."];
Print["==CACHE== ", cache];
