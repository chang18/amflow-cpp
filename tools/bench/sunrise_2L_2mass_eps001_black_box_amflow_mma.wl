...
(*
  2-loop sunrise with 2 distinct masses on l1 and l2 (off-shell external).
*)
...
targets = { j[sunrise2m, 1, 1, 1, 0, 0] };
epslist = {1/1000};
cache = Environment["AMF_BENCH_MMA_CACHE"];
If[cache === $Failed || cache === None,
  cache = FileNameJoin[{repo, "tools", "bench", "mma_refs",
    "sunrise_2L_2mass_eps001_mma_cache"}]];

Print["==BENCH== mma 2-loop sunrise with 2 distinct masses on l1 and l2 (off-shell external)."];
Print["==CACHE== ", cache];
