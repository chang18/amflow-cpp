...
(*
  1-loop triangle with 2 distinct internal masses (mAsq on l^2,
  mBsq on (l+p1)^2; the third propagator (l+p1+p2)^2 remains
  massless).  Target: corner j[tri2m, 1, 1, 1] at eps = 1/1000.
*)
...
AMFlowInfo["Numeric"] = {s -> -3, mAsq -> 1, mBsq -> 4};
AMFlowInfo["NThread"] = 4;

targets = { j[tri2m, 1, 1, 1] };
epslist = {1/1000};
cache = Environment["AMF_BENCH_MMA_CACHE"];
If[cache === $Failed || cache === None,
  cache = FileNameJoin[{repo, "tools", "bench", "mma_refs",
    "triangle_1L_2mass_mma_cache"}]];

Print["==BENCH== mma 1-loop triangle with 2 distinct masses"];
Print["==CACHE== ", cache];
