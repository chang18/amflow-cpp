...
(*
  2-loop sunset with 2 distinct internal masses on the first two
  loop propagators (mAsq on l1, mBsq on l2).  Target: j[sunset2m,
  1, 1, 1, 0, 0] at eps = 1/1000.
*)
...
AMFlowInfo["Numeric"] = {s -> 1, mAsq -> 1, mBsq -> 4};
AMFlowInfo["NThread"] = 4;

targets = { j[sunset2m, 1, 1, 1, 0, 0] };
epslist = {1/1000};
cache = Environment["AMF_BENCH_MMA_CACHE"];
If[cache === $Failed || cache === None,
  cache = FileNameJoin[{repo, "tools", "bench", "mma_refs",
    "sunset_2L_2mass_eps001_mma_cache"}]];

Print["==BENCH== mma 2-loop sunset with 2 distinct masses"];
Print["==CACHE== ", cache];
